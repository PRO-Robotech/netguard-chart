{{- define "netguard.web-resources.factory.manifets.networkbinding-details" -}}
{{- $key          := (default "networkbinding-details" .key) -}}
                                  ...
                                  - type: Labels
                                    data:
                                      containerStyle:
                                        marginTop: -30px
                                      editModalWidth: 650
                                      endpoint: "/api/clusters/{{ $pathIndexCluster }}/k8s/apis/netguard.sgroups.io/v1beta1/namespaces/{{ $pathIndexNamespace }}/networkbindings/{{ $pathIndexResName }}"
                                      id: labels-editor
                                      inputLabel: ""
                                  ...
                                  - type: antdFlex
                                    ...
                                    children:
                                      - type: ResourceBadge
                                        data:
                                          id: ns-badge
                                          value: "{reqsJsonPath[0]['.items.0.spec.networkRef.kind']['-']}"
                                      - type: antdLink
                                        data:
                                          id: ns-link
                                          href: "/{{ $pathIndexBasePrefix }}/{{ $pathIndexCluster }}/{reqsJsonPath[0]['.items.0.spec.networkRef.namespace']['-']}/factory/network-details/netguard.sgroups.io/v1beta1/networks/{reqsJsonPath[0]['.items.0.spec.networkRef.name']['-']}"
                                        {{ if eq ($nsAliasMode | toString) "true" }}
                                          text: |
                                            {reqsJsonPath[1]['.items[?(@.metadata.name=="{reqsJsonPath[0][".items.0.spec.networkRef.namespace"]}")].metadata.labels.alias']["-"]}/{reqsJsonPath[0][".items.0.spec.networkRef.name"]["-"]}
                                        {{ else }}
                                          text: '{reqsJsonPath[0][".items.0.spec.networkRef.namespace"]["-"]}/{reqsJsonPath[0][".items.0.spec.networkRef.name"]["-"]}'
                                        {{ end }}

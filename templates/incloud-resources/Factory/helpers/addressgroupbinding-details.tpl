{{- define "netguard.web-resources.factory.manifets.addressgroupbinding-details" -}}
{{- $key          := (default "addressgroupbinding-details" .key) -}}
                                  ...
                                  - type: Labels
                                    data:
                                      containerStyle:
                                        marginTop: -30px
                                      editModalWidth: 650
                                      endpoint: "/api/clusters/{{ $pathIndexCluster }}/k8s/apis/netguard.sgroups.io/v1beta1/namespaces/{{ $pathIndexNamespace }}/addressgroupbindings/{{ $pathIndexResName }}"
                                      id: labels-editor
                                      inputLabel: ""
                                  ...
                                  - type: antdFlex
                                    ...
                                    children:
                                      - type: ResourceBadge
                                        data:
                                          id: ns-badge
                                          value: "{reqsJsonPath[0]['.items.0.spec.serviceRef.kind']['-']}"
                                      - type: antdLink
                                        data:
                                          id: ns-link
                                          href: "/{{ $pathIndexBasePrefix }}/{{ $pathIndexCluster }}/{reqsJsonPath[0]['.items.0.spec.serviceRef.namespace']['-']}/factory/hbf-service-details/netguard.sgroups.io/v1beta1/services/{reqsJsonPath[0]['.items.0.spec.serviceRef.name']['-']}"
                                        {{ if eq ($nsAliasMode | toString) "true" }}
                                          text: |
                                            {reqsJsonPath[1]['.items[?(@.metadata.name=="{reqsJsonPath[0][".items.0.spec.serviceRef.namespace"]}")].metadata.labels.alias']["-"]}/{reqsJsonPath[0][".items.0.spec.serviceRef.name"]["-"]}
                                        {{ else }}
                                          text: '{reqsJsonPath[0][".items.0.spec.serviceRef.namespace"]["-"]}/{reqsJsonPath[0][".items.0.spec.serviceRef.name"]["-"]}'
                                        {{ end }}
            ...
            children:
              - type: YamlEditorSingleton
                data:
                  ...
                  plural: addressgroupbindings
                  pathToData: .items.0
                  forcedKind: AddressGroupBinding
                  apiGroup: netguard.sgroups.io
                  apiVersion: v1beta1
{{ end }}
